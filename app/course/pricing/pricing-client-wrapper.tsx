...
export function PricingClientWrapper({
  ...
  const relevantBundles = bundleTiers.filter((p) =>
    isBundleRelevant(p.key, currentAccess),
  );

  // Filter platform tiers to only show ones user doesn't own
  const availablePlatformTiers = platformTiers.filter(
    (p) => !isProductOwned(p.key, currentAccess),
  );

  // Check if user has any paid access (for messaging)
  const hasPartialAccess =
    currentAccess && currentAccess !== "free" && currentAccess !== "full";
    ...
    <>
      ...
      {showValueBanner && (
        <div className="border-swiss-red bg-swiss-red/2.5 dark:bg-swiss-red/5 mb-12 rounded-none border-2 p-8">
          <div className="mx-auto max-w-3xl text-center">
            <div className="border-swiss-red text-swiss-red mb-4 inline-flex items-center gap-2 border-2 bg-white px-4 py-2 text-sm font-bold dark:bg-neutral-900">
              <Crown className="h-4 w-4" />
              Best Value
            </div>
            <h2 className="heading-section mb-4">
              <span className="text-swiss-red">Convergence: All-Access</span> =
              Everything Included
            </h2>
            <p className="mb-6 text-neutral-600 dark:text-neutral-400">
              Get complete access to all Design, Engineering, and exclusive
              Convergence content across Web, iOS, and Android — at a fraction
              of the cost of buying tracks individually.
            </p>
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="border-swiss-red rounded-none border-2 bg-white p-6 dark:bg-neutral-900">
                ...
                <p className="text-xs font-bold text-green-600 dark:text-green-400">
                  SAVE {convergenceSavingsPercent}% (
                  {formatGBP(convergenceSavings)})
                </p>
              </div>
            </div>
            <div className="mt-6 text-sm text-neutral-600 dark:text-neutral-400">
              💡 <strong>Students save even more</strong> — scroll down for 30%
              off
            </div>
          </div>
        </div>
      ...
      {relevantBundles.length > 0 && (
        <div className="mb-8">
          ...
          <div
            className={`grid gap-8 ${relevantBundles.length === 1 ? "mx-auto max-w-md" : relevantBundles.length === 2 ? "mx-auto max-w-3xl md:grid-cols-2" : "md:grid-cols-3"}`}
          >
      ...
      {availablePlatformTiers.length > 0 && (
        <div className="mt-16 mb-16">
          ...
          <div
            className={`grid gap-4 ${availablePlatformTiers.length <= 2 ? "mx-auto max-w-2xl sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`}
          >
