...
export default async function PricingPage() {
  ...
  const bundleTiers = BUNDLE_TIER_KEYS.map((key) =>
    allProducts.find((p) => p.key === key),
  ).filter((p): p is ProductWithPrice => p !== undefined);

  // Individual platform tracks
  const platformTiers = PLATFORM_TIER_KEYS.map((key) =>
    allProducts.find((p) => p.key === key),
  ).filter((p): p is ProductWithPrice => p !== undefined);
  ...
  const individualTrackTotal = platformTiers.reduce(
    (sum, p) => sum + p.price,
    0,
  );
  const convergenceSavings =
    individualTrackTotal > 0 && fullProduct
      ? individualTrackTotal - fullProduct.price
      : 0;
  const convergenceSavingsPercent =
    individualTrackTotal > 0 && fullProduct
      ...
      <div className="container-page py-12">
        ...
        {hasFullAccess ? (
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h1 className="heading-section mb-4 md:text-4xl">
              You Have Full Access!
            </h1>
            <p className="mb-8 text-lg text-neutral-600 dark:text-neutral-400">
              Thank you for being a Convergence student. You have access to
              everything—all tracks, all platforms, and all exclusive content.
            ...
            <div className="rounded-none border border-green-200 bg-green-50 p-6 dark:border-green-800 dark:bg-green-950/30">
              <p className="text-green-800 dark:text-green-200">
                ...
                <strong>Convergence: All-Access</strong> — Lifetime access to
            ...
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:justify-center">
              <Link
                href="/course"
                className="bg-swiss-red inline-flex items-center justify-center gap-2 px-6 py-3 font-medium text-white transition-colors hover:bg-neutral-900 dark:hover:bg-white dark:hover:text-black"
              >
                Continue Learning
              </Link>
              <Link
                href="/course/dashboard"
                className="hover:border-swiss-red hover:text-swiss-red dark:hover:border-swiss-red inline-flex items-center justify-center gap-2 border border-neutral-300 bg-white px-6 py-3 font-medium transition-colors dark:border-neutral-700 dark:bg-neutral-800"
              >
                View Dashboard
              </Link>
            </div>
          ...
          <>
            ...
            <div className="mb-12 text-center">
              <h1 className="heading-section mb-4 md:text-4xl md:text-5xl">
                Invest in Your Future
              </h1>
              <p className="mx-auto max-w-2xl text-lg text-neutral-600 dark:text-neutral-400">
                Choose the path that fits your goals. All plans include lifetime
            ...
            <div className="rounded-none border border-neutral-200 bg-white p-8 text-center dark:border-neutral-800 dark:bg-neutral-900">
              <div className="bg-swiss-red/10 mx-auto mb-4 flex h-12 w-12 items-center justify-center">
                <ShieldCheck className="text-swiss-red h-6 w-6" />
              </div>
              ...
              <p className="mx-auto max-w-xl text-neutral-600 dark:text-neutral-400">
                ...
                satisfied within 14 days of purchase, email us and we&apos;ll
                refund you in full—no questions asked.
              </p>
              <Link
                href="/course/refund-policy"
                className="text-swiss-red mt-4 inline-block text-sm hover:underline"
              >
