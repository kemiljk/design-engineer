...
export function StudentDiscountForm() {
  ...
  return (
    <div className="border-swiss-red/20 bg-swiss-red/2.5 dark:border-swiss-red/30 dark:bg-swiss-red/5 rounded-none border-2 p-8">
      <div className="mx-auto max-w-xl">
        <div className="mb-4 flex items-center justify-center">
          <div className="bg-swiss-red flex h-12 w-12 items-center justify-center text-white">
        ...
        <p className="mb-6 text-center text-sm text-neutral-600 dark:text-neutral-400">
          Currently enrolled in university or college? Get an instant discount
          code sent to your student email.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={status === "loading" || status === "success"}
            className="bg-swiss-red flex w-full items-center justify-center gap-2 px-6 py-3 font-medium text-white transition-colors hover:bg-neutral-900 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-white dark:hover:text-black"
          >
            {status === "loading" ? (
              <>
            ...
            <div className="flex-1">
              ...
              {status === "success" && (
                <p className="mt-2 text-xs opacity-90">
                  Check your inbox (and spam folder). Your unique discount code
                  will be waiting for you!
                </p>
              )}
              {status === "error" &&
                message.includes("not being recognised") && (
                  <p className="mt-2 text-xs opacity-90">
                    Contact us at{" "}
                    <a
        ...
        <div className="mt-6 rounded-none border border-neutral-200 bg-white p-4 text-sm dark:border-neutral-800 dark:bg-neutral-900">
          ...
          <ol className="ml-4 list-decimal space-y-1 text-neutral-600 dark:text-neutral-400">
            ...
            <li>
              We&apos;ll verify it&apos;s from a recognised educational
              institution
            </li>
        ...
        <p className="mt-4 text-center text-xs text-neutral-500">
          Your email is only used to verify student status and send your
          discount code. We don&apos;t share it with anyone.
        </p>
      </div>
    </div>
  );
