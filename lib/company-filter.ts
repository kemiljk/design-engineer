/**
 * Company filtering logic for student companies feature
 * Filters out personal email providers and personal domains to only show legitimate companies
 */
...
export const PERSONAL_TLDS = new Set([
...
export const EXCLUDED_SPECIFIC_DOMAINS = new Set([
...
export const MIN_STUDENTS_THRESHOLD = 2;
...
/**
 * Check if a domain should be excluded from company display
 */
export function shouldExcludeDomain(domain: string, studentCount: number): boolean {
  const lowerDomain = domain.toLowerCase();
